...
delete from sheikhs
 where slug in ('abdullah-almohammed','sulaiman-alfahd','ahmed-alnasser')
   and id in (select sheikh_id from removed);
*/

-- اللقاءات تُحذف تلقائياً مع سلاسلها (on delete cascade — الأثر الجانبي ٨.٢).
-- شرط «and id in (select ...)» حارس: إن كنتَ أنشأت سلسلة حقيقية تحت أحد هؤلاء
-- المشايخ فسيفشل الأمر بخطأ مفتاح أجنبي ولن يُحذف شيء — وهذا مقصود.
