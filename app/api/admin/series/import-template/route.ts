 ...
 *
 * صفّ واحد = لقاء واحد؛ صفوف السلسلة نفسها (رابط واحد) تتكرّر فيها
 * أعمدة العنوان/الشيخ/النوع... حرفياً، ويختلف فيها التاريخ والوقت وحدهما.
 * هذا يطابق تماماً ما يتحقّق منه `lib/server/excel-import.ts` عند الرفع.
 */
...
export async function GET() {
  ...
  const lines = [
    ...
    '٢) لسلسلة من عدّة لقاءات: كرّر رابط السلسلة نفسه في كل صفوفها، بنفس العنوان والشيخ والنوع والمكان والروابط والمدة حرفياً — ويختلف التاريخ والوقت وحدهما من صفّ لآخر.',
    '٣) رابط السلسلة يجب أن يكون جديداً تماماً — غير مستخدَم لأي سلسلة موجودة حالياً في المنصة.',
    '٤) عمود «رابط الشيخ» فيه قائمة منسدلة بروابط المشايخ النشطين وقت تنزيل هذا القالب. أضف شيخاً جديداً من تبويب «المشايخ» في اللوحة أولاً إن لم يكن ضمنها.',
    '٥) عمود «النوع» قائمة منسدلة بثلاث قيم فقط: حضوري / عن بُعد / حضوري وعن بُعد.',
    '٦) «رابط الدخول» مطلوب إن كان النوع غير حضوري بحت.',
    '٧) صيغة التاريخ: سنة-شهر-يوم مثل 2026-09-15 — وصيغة الوقت: ساعة:دقيقة على مدار ٢٤ ساعة مثل 20:00.',
    '٨) الملف يُفحَص كاملاً قبل أي حفظ: أي خطأ في أي صفّ يمنع استيراد الملف كله، وتظهر لك قائمة بكل الأخطاء دفعة واحدة لتصحيحها معاً.',
    '',
  ...
  for (let r = 2; r <= LAST_ROW; r++) {
    ...
    wsData.getCell(`E${r}`).dataValidation = {
      type: 'list',
      allowBlank: true,
