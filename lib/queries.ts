...
/**
 * هوية صفحة الشيخ — **من لقطة اللقاء بعد التغليب، لا من السلسلة وحدها**.
 *
 * هذا هو مربط النموذج القالبي، ممدوداً لمستوى اللقاء (هجرة ٠٠٣): شيخ لا
 * يظهر إلا متجاوِزاً على لقاء بعينه (لا شيخاً افتراضياً لأي سلسلة —
 * حال أغلب المشايخ في سلسلة كثيرة التناوب) يجب أن تعمل صفحته أيضاً، لا
 * `series.sheikh_slug` وحده الذي يفوته تماماً. `v_lectures` يحمل التغليب
 * جاهزاً (`coalesce(l.sheikh_slug, s.sheikh_slug)`) فيكفي استعلام واحد عليه.
 *
 * ويعيد `null` — أي ٤٠٤ — إن لم يوجد له لقاء ظاهر أصلاً، فلا يُنشَر رابط
